...
public class GameScreen extends JPanel {
	...
	public void updateScreen(){
		...
		if(status.isGameStarting()){
			soundMan.playdeadmau5Sound();
			drawGetReady();
			return;
		}
		if(status.getAsteroidsDestroyed() != 0 && !status.isGameOver()){
			if(status.getAsteroidsDestroyed() % 5 == 0){
				drawGetReadyNextLevel();
			}
		}
		...
		if(!status.isNewAsteroid()){
			...
			if(asteroid.getY() + asteroid.getSpeed() < this.getHeight()){
				if(status.getAsteroidsDestroyed() > 5){
					if(status.getAsteroidsDestroyed() > 10){
						for(int i=0; i<asteroids.size(); i++){
							Asteroid newAsteroid = asteroids.get(i);
							newAsteroid.translate(0, rand.nextInt(newAsteroid.getSpeed()));
							graphicsMan.drawAsteroid(newAsteroid, g2d, this);
						}
					}
					asteroid.translate((int)Math.pow(-1, status.getAsteroidsCreated())*asteroid.getSpeed(),rand.nextInt(asteroid.getSpeed()));
				...
				status.setAsteroidsCreated(status.getAsteroidsCreated() + 1);
	...
	/**
	 * Draws "Get Ready for the Next Level!" message.
	 */
	private void drawGetReadyNextLevel() {
		String readyStr = "Get Ready for the Next Level!";
		g2d.setFont(originalFont.deriveFont(originalFont.getSize2D() + 1));
		FontMetrics fm = g2d.getFontMetrics();
		int ascent = fm.getAscent();
		int strWidth = fm.stringWidth(readyStr);
	...
	/**
	 * Sets the label that displays the value for the current score
